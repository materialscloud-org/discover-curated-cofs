...
def plot_isotherm(**kwargs):
    ...
    for gas in ['co2', 'n2']:
        ...
        p1.circle('p', 'q_avg', source=source, color=plotcolor[gas], size=10, legend_label=plotlabel[gas])
        p1.add_layout(bmd.Whisker(source=source, base="p", upper="q_upper", lower="q_lower")  #, level="overlay")
                     )
        ...
        p2.add_layout(
            bmd.Whisker(source=source, base="h_avg", upper="h_upper", lower="h_lower",
                        dimension='width')  # , level="overlay")
        )
