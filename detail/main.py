...
def get_label():
    ...
    try:
        ...
        #structure_label = '05000N2' # example v1, nonporous
        #structure_label = '05001N2' # example v1
        #structure_label = '19353N2'  # example v2
        structure_label = '20091N2'  # example v3
...
class DetailView():
    ...
    def __init__(self):
        ...
        self.group_dict = get_group_as_dict(self.cof_label)  # get linking group

    def iso_plot(self):
        from detail.plot_isotherm import plot_isotherm
        pm_co2 = self.group_dict['isot_co2_out']
        pm_n2 = self.group_dict['isot_n2_out']
        mpl_fig = plot_isotherm(pm_co2=pm_co2, pm_n2=pm_n2, label=self.cof_label, version=self.group_dict['version'])
        column = pn.Column(get_title("Isotherms for " + self.cof_label, uuid=pm_co2.uuid))
        column.append(pn.pane.Bokeh(mpl_fig))
        return column

    def ener_plot(self):
        from detail.plot_cp2k import plot_energy_steps
        dftopt_node = self.group_dict['dftopt_out']  #Singlefile (v1) or Dict (v2)
        column = pn.Column(get_title("Robust cell optimization of " + self.cof_label, uuid=dftopt_node.uuid))
        column.append(pn.pane.Bokeh(plot_energy_steps(dftopt_node, self.cof_label, self.group_dict['version'])))
        return column

    def structure_opt(self):
        from detail.structure import structure_jsmol
        cif_node = self.group_dict['opt_cif_ddec']
        column = pn.Column(get_title("DFT-optimized structure for " + self.cof_label, uuid=cif_node.uuid))
        column.append(pn.pane.Bokeh(structure_jsmol(cif_str=cif_node.get_content())))
        return column

    def process(self):
        from detail.process import print_process
        process_node = self.group_dict['pe_out']
        column = pn.Column(get_title("Process sketch for " + self.cof_label, uuid=process_node.uuid))
